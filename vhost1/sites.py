...
def ftp(stack_name,file_url,site_content,project_name,timestamp):
    with open('/tmp/backup_sites.txt', 'a') as f:
        f.write(f"{stack_name}\t\t{file_url}\t\t{site_content}\t\t{project_name}\t\t{timestamp}\n")
    session = ftplib.FTP('backup', 'ftp', 'ftp')
    file = open('/tmp/backup_sites.txt','rb')
    session.storbinary('STOR backup_sites.txt', file)
    file.close()
    session.quit()
...
def create_site():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            cursor.execute("""INSERT into SITES (STACK_NAME, FILE_URL, SITE_CONTENT, PROJECT_NAME, DATE_CREATED) values(%s,%s,%s,%s,%s)""",(stack_name,file_url,site_content,project_name,timestamp))
            connection.commit()
            connection.close()
            ftp(stack_name,file_url,site_content,project_name,timestamp)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"Site '{stack_name}' successfully updated!",
                  category="success")
            cursor.execute(("""UPDATE SITES SET FILE_URL = %s, SITE_CONTENT = %s, PROJECT_NAME = %s  WHERE STACK_NAME = %s"""),(file_url,site_content,project_name,stack_name))
            connection.commit()
            connection.close()
...
def delete_site(id: str):
    ...
    try:
        ...
        cursor.execute(("DELETE FROM SITES WHERE STACK_NAME = %s"), (stack_name))
        connection.commit()
        connection.close()
    except auto.ConcurrentUpdateError:
        flash(
