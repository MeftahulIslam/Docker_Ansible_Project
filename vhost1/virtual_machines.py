...
with open('/tmp/backup_vm.txt', 'a') as f:
    f.write("stack_name\t\tproject_name\t\tinstance_type\t\tkeydata\t\ttimestamp\n")

def ftp(stack_name,project_name,instance_type,keydata,timestamp):
    with open('/tmp/backup_vm.txt', 'a') as f:
        f.write(f"{stack_name}\t\t{project_name}\t\t{instance_type}\t\t{keydata}\t\t{timestamp}\n")
    session = ftplib.FTP('backup', 'ftp', 'ftp')
    file = open('/tmp/backup_vm.txt','rb')
    session.storbinary('STOR backup_vm.txt', file)
    file.close()
    session.quit()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(
                ...
                #here insert into
            cursor.execute("""INSERT into VM (STACK_NAME, PROJECT_NAME, INSTANCE_TYPE, KEYDATA, DATE_CREATED) values(%s,%s,%s,%s,%s)""",(stack_name,project_name,instance_type,keydata,timestamp))
            connection.commit()
            connection.close()
            ftp(stack_name,project_name,instance_type,keydata,timestamp)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"VM '{stack_name}' successfully updated!",
                  category="success")
            cursor.execute(("""UPDATE VM SET PROJECT_NAME = %s, INSTANCE_TYPE = %s, KEYDATA = %s  WHERE STACK_NAME = %s"""),(project_name,instance_type,keydata,stack_name))
            connection.commit()
            connection.close()
...
def delete_vm(id: str):
    ...
    try:
        ...
        cursor.execute(("DELETE FROM VM WHERE STACK_NAME = %s"), (stack_name))
        connection.commit()
        connection.close()
    except auto.ConcurrentUpdateError:
        flash(
